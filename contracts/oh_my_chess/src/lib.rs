    ...
    impl OhMyChess {
        ...
        pub fn new() -> Result<Self> {
            let instance = Self {
                ...
                url: String::from("https://eu-central-1.aws.data.mongodb-api.com/app/data-bluba/endpoint/data/v1"),
                api_key: String::from("Xq8d8xug4fxn9mz9JERMR4D72GhDRn5M2SUK49G4qjJ2FlaXd0NPptKLzPoyK9bN"),
            };
            Ok(instance)
        }

        #[ink(message)]
        pub fn start_new_game_session(&self) -> Result<String> {
            let player_black: [u8; 32] = *(Self::env().caller().as_ref());
            let player_white: [u8; 32] = *(Self::env().caller().as_ref());
        ...
        #[ink(message)]
        pub fn make_move(&self) -> Result<()> {
            let chess_move: ChessMove = ChessMove{ from: (1, 2), to: (3, 2)};
            let session_id: String = String::from("65ee5d8a95f3de8dcf8d091f");
        ...
        pub fn update_players_sessions_track_in_mongodb(&self, session_id: heapless::String<32>, player_address: [u8; 32]) -> Result<()> {
            ...
            pink::warn!("-----SESSION_ID FORMAT {}", session_id.clone());
            pink::warn!("-----URL FORMAT {}", url.clone());
            ...
            // let data_format: core::result::Result<heapless::String<8192>, _> = serde_json_core::ser::to_string(data.as_slice());
            // pink::warn!("-----UPDATE DATA FORMAT\n {:?}", data_format);
        ...
        pub fn insert_game_session_to_mongodb(&self, game_state: GameState) -> Result<heapless::String<32>> {
            ...
            // pink::warn!("-----RESPONSE\n {:?}", response.body.clone());
            // let data_format: core::result::Result<heapless::String<8192>, _> = serde_json_core::ser::to_string(data.as_slice());
            // pink::warn!("-----DATA FORMAT\n {:?}", data_format);

            // let response_format: core::result::Result<heapless::String<1024>, _> = serde_json_core::ser::to_string(response.body.clone().as_slice());
            // pink::warn!("-----RESPONSE FORMAT\n {:?}", response_format);
